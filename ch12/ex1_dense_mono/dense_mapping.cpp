...
bool epipolarSearch(
    ...
    // 修正：不要 normalize()，保持归一化坐标形式 [u', v', 1]^T
    Vector3d f_ref = px2cam(pt_ref);      
    Vector3d P_ref = f_ref * depth_mu;    // 注意：这里 P_ref.z() = depth_mu
    
    Vector2d px_mean_curr = cam2px(T_C_R * P_ref);
    double d_min = depth_mu - 3 * depth_cov, d_max = depth_mu + 3 * depth_cov;
    if (d_min < 0.1) d_min = 0.1;
    
    // 使用相同的 f_ref（未归一化）计算极线端点
...
bool updateDepthFilter(
    ...
    if (isnan(ans[0]) || isnan(ans[1]) || isinf(ans[0]) || isinf(ans[1])) {
        return false;
    }
    ...
    double depth_estimation = p_esti.z();
    
    if (depth_estimation <= 0.1 || isnan(depth_estimation) || isinf(depth_estimation)) {
        return false;
    }
    ...
    // 【关键修复】限制 acos 参数在 [-1, 1] 范围内
    double cos_alpha = f_ref.dot(t) / t_norm;
    cos_alpha = std::max(-0.999999, std::min(0.999999, cos_alpha));
    double alpha = acos(cos_alpha);
    
    double cos_beta = -a.dot(t) / (a_norm * t_norm);
    cos_beta = std::max(-0.999999, std::min(0.999999, cos_beta));
    double beta = acos(cos_beta);
    
    // 计算单位像素误差
    Vector3d f_curr_prime = px2cam(pt_curr + epipolar_direction);
    
    double cos_beta_prime = f_curr_prime.dot(-t) / t_norm;
    cos_beta_prime = std::max(-0.999999, std::min(0.999999, cos_beta_prime));
    double beta_prime = acos(cos_beta_prime);
    
    double gamma = M_PI - alpha - beta_prime;
    
    // 检查 sin(gamma) 是否接近 0
    double sin_gamma = sin(gamma);
    if (fabs(sin_gamma) < 1e-6) {
        // 如果 sin(gamma) 太小，使用备选方案
        return false;
    }
    
    double p_prime = t_norm * sin(beta_prime) / sin_gamma;
    double d_cov = p_prime - depth_estimation;
    double d_cov2 = d_cov * d_cov;
    
    if (isnan(d_cov2) || isinf(d_cov2) || d_cov2 < 0) {
        d_cov2 = 1.0;
    }

    // 高斯融合
    int u = int(pt_ref(0, 0));
    int v = int(pt_ref(1, 0));
    
    if (u < 0 || u >= depth.cols || v < 0 || v >= depth.rows) {
        return false;
    }
    
    double mu = depth.ptr<double>(v)[u];
    double sigma2 = depth_cov2.ptr<double>(v)[u];
    ...
    if (isnan(sigma2) || isinf(sigma2) || sigma2 < 0) {
        sigma2 = d_cov2;
    }
    
    double denominator = sigma2 + d_cov2;
    if (denominator < 1e-10) {
        return false;
    }
    
    double mu_fuse = (d_cov2 * mu + sigma2 * depth_estimation) / denominator;
    double sigma_fuse2 = (sigma2 * d_cov2) / denominator;
    
    if (isnan(mu_fuse) || isinf(mu_fuse)) {
        return false;
    }
...
bool updateDepthFilter(
    ...
    // u = pt_ref(0, 0) （列坐标，x方向）
    // v = pt_ref(1, 0) （行坐标，y方向）
...
void evaludateDepth(const Mat &depth_truth, const Mat &depth_estimate) {
    ...
    for (int y = boarder; y < depth_truth.rows - boarder; y++) {
        for (int x = boarder; x < depth_truth.cols - boarder; x++) {
            double truth = depth_truth.ptr<double>(y)[x];
            double estimate = depth_estimate.ptr<double>(y)[x];
            
            // 跳过无效值
            if (truth < 0.1) continue;
            if (isnan(estimate) || isinf(estimate)) continue;
            
            double error = truth - estimate;
            ave_depth_error += error;
            ave_depth_error_sq += error * error;
            cnt_depth_data++;
        }
