...
// tj : https://ceres-solver.googlesource.com/ceres-solver/+/master/examples/simple_bundle_adjuster.cc
...
int main(int argc, char **argv)
{
  if (argc != 5)
  {
    cout << "usage: pose_estimation_3d2d img1 img2 depth1 depth2" << endl; // tj : here doesn't use depth2, we use depth1 to recover 3d points as in the world coordinate
    return 1;
  }
  //-- 读取图像
    ...
    Sophus::SE3d &pose)
{
  ...
  for (int iter = 0; iter < iterations; iter++)
  {
    ...
    for (int i = 0; i < points_3d.size(); i++)
    {
      ...
      cost += e.squaredNorm();
      Eigen::Matrix<double, 2, 6> J; // tj : 这里只有pose的雅可比，因为这里我们只优化pose，3d点不变
      J << -fx * inv_z,
          0,
          fx * pc[0] * inv_z2,
          fx * pc[0] * pc[1] * inv_z2,
